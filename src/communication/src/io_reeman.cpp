...
#define ENDPOINT_GET_LASER_DATA "/reeman/laser"
...
#define FSM_INIT 0
#define FSM_SAFEOP 1
#define FSM_SAFEOP_HANDTRACK 5
#define FSM_OPERATION 2
#define FSM_DOING_CHARGING 3
#define FSM_TEST_MULTIROBOT 4
#define FSM_HUMAN_INTERVENTION 6
#define FSM_LAUNCHING 7
...
public:
    ...
    rclcpp::TimerBase::SharedPtr tim_routine_normal;

    HelpLogger logger;
    MachineState fsm_robot;

    // Configs
    std::string reeman_controller_ip = "10.7.101.107";
    // std::string reeman_controller_ip = "";
    float target_point_1_x = 0;
    float target_point_1_y = 0;
    float target_point_1_yaw = 0;

    float target_point_2_x = 0;
    float target_point_2_y = 0;
    float target_point_2_yaw = 0;
    ...
    // lidar vars
    obstacle_checking_t obs_front_near;
    obstacle_checking_t obs_front_far;
    obstacle_checking_t obs_cone_near;

    std::vector<point2d_t> lidar_points;
    ...
    uint64_t normal_global_counter_ms = 0;
    uint64_t last_time_request_counter_ms = 0;
    uint64_t last_time_connected_ds4 = 0;
    ...
    // Gesture Input
    rclcpp::Subscription<std_msgs::msg::Int16MultiArray>::SharedPtr sub_hand_gesture;

    int16_t hand_gesture = 0;
    int16_t hand_position = 0;
    int16_t used_hand_gesture = 0;
    int16_t used_hand_position = 0;

    // face input
    rclcpp::Subscription<std_msgs::msg::Int8>::SharedPtr sub_face_detected;

    int8_t face_detected = 0;

    // UI Input
    rclcpp::Subscription<std_msgs::msg::Int8>::SharedPtr sub_goto_docking;
    ...
    rclcpp::Publisher<std_msgs::msg::Int8>::SharedPtr pub_docking_status;
    rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr pub_robot_battery_status;
    rclcpp::Publisher<std_msgs::msg::Int8>::SharedPtr pub_moving_status;
    rclcpp::Publisher<std_msgs::msg::Int8>::SharedPtr pub_hand_audio;
    rclcpp::Publisher<std_msgs::msg::Int8>::SharedPtr pub_halo_audio;

    // rclcpp::Publisher<std_msgs::msg::Int8>::SharedPtr pub_audio_hand;
    ...
    uint8_t counter_kirim_motion_0 = 0;

    // FLAGS
    bool motion_to_point_canceled = false;
    bool docking_canceled = false;
    IOReeman()
        : Node("io_reeman")
    {
        ...
        this->declare_parameter("target_point_1_x", target_point_1_x);
        this->get_parameter("target_point_1_x", target_point_1_x);

        this->declare_parameter("target_point_1_y", target_point_1_y);
        this->get_parameter("target_point_1_y", target_point_1_y);

        this->declare_parameter("target_point_2_yaw", target_point_2_yaw);
        this->get_parameter("target_point_2_yaw", target_point_2_yaw);

        this->declare_parameter("target_point_2_x", target_point_2_x);
        this->get_parameter("target_point_2_x", target_point_2_x);

        this->declare_parameter("target_point_2_y", target_point_2_y);
        this->get_parameter("target_point_2_y", target_point_2_y);

        this->declare_parameter("target_point_1_yaw", target_point_1_yaw);
        this->get_parameter("target_point_1_yaw", target_point_1_yaw);
        ...
        rclcpp::SubscriptionOptions options_timer;

        options.callback_group = callback_group_subscribers_;
        options_timer.callback_group = timer_group_;

        sleep(2);
        sub_ds4 = this->create_subscription<ds4_driver_msgs::msg::Status>("/ds4/status", 1, std::bind(&IOReeman::callback_ds4, this, std::placeholders::_1), options);
        sub_keyboard_hit = this->create_subscription<std_msgs::msg::Int16>("/key_pressed", 1, std::bind(&IOReeman::callback_kbhit, this, std::placeholders::_1), options);
        sub_terminal_update = this->create_subscription<ros2_interface::msg::TerminalArray>("/basestation/terminal_update", 1, std::bind(&IOReeman::callback_terminal_update, this, std::placeholders::_1), options);
        sub_face_detected = this->create_subscription<std_msgs::msg::Int8>("/vision/face_detected", 1, std::bind(&IOReeman::callback_face_detected, this, std::placeholders::_1), options);
        sub_hand_gesture = this->create_subscription<std_msgs::msg::Int16MultiArray>("/vision/hand_data", 1, std::bind(&IOReeman::callback_hand_gesture, this, std::placeholders::_1), options);
        sub_goto_docking = this->create_subscription<std_msgs::msg::Int8>("/ui/goto_docking", 1, std::bind(&IOReeman::callback_ui_goto_docking, this, std::placeholders::_1), options);

        pub_robot_array = this->create_publisher<ros2_interface::msg::RobotArray>("/communication/robot_array", 1);
        pub_docking_status = this->create_publisher<std_msgs::msg::Int8>("/communication/docking_status", 1);
        pub_robot_battery_status = this->create_publisher<std_msgs::msg::Float32>("/communication/robot_battery_status", 1);
        pub_moving_status = this->create_publisher<std_msgs::msg::Int8>("/communication/moving_status", 1);
        pub_hand_audio = this->create_publisher<std_msgs::msg::Int8>("/vision/hand_action_ready", 1);
        pub_halo_audio = this->create_publisher<std_msgs::msg::Int8>("/vision/play_greeting", 1); //

        tim_routine = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&IOReeman::callback_routine, this));
        // tim_routine_normal = this->create_wall_timer(std::chrono::milliseconds(200), std::bind(&IOReeman::callback_routine_normal, this));
    ...
    void callback_face_detected(std_msgs::msg::Int8::SharedPtr msg)
    {
        static int64_t last_time_face_detected = 0;
        static int8_t prev_face_detected = 0;
        face_detected = msg->data;

        if (face_detected == 1 && prev_face_detected == 0)
        {

            std_msgs::msg::Int8 msg_halo_audio;
            msg_halo_audio.data = 1;
            pub_halo_audio->publish(msg_halo_audio);

            last_time_face_detected = global_counter_ms;
        }

        prev_face_detected = face_detected;

        if (face_detected == 1 && obs_front_near.status == 1 && obs_front_near.distance < 0.6f)
        {

            // logger.warn("Obstacle detected in front while face detected, assume it's a human, stopping robot");
            // send_stop_command();
            // motion_cancel();
            // fsm_robot.value = FSM_HUMAN_INTERVENTION;
        }

        //? SOLVE DISPLAY
        static uint8_t set_display_mode = 0;

        if (set_display_mode == 0 && normal_global_counter_ms > 11000)
        {
            logger.info("Setting DISPLAY mode for xdotool...");
            system("export DISPLAY=:0 && sleep 1 && xdotool mousemove 500 300 click 1");
            set_display_mode = 1;
        }
    ...
    {
        static int16_t prev_hand_gesture = -1;
        static int16_t cntr_hand_detected = 0;

        hand_gesture = msg->data[0];
        hand_position = msg->data[1];

        if ((hand_gesture != -1 && hand_position != 0) && prev_hand_gesture != hand_gesture)
        {
            ...
            pub_hand_audio->publish(msg_hand_audio);
        }

        prev_hand_gesture = hand_gesture;
    }

    void motion_to_point_hand(int8_t gesture)
    {

        logger.info("%.2f %.2f %.2f | Gesture: %d | Hand Pos: %d", final_pose_x, final_pose_y, final_pose_yaw, gesture, hand_position);

        static uint64_t last_time_motion = 0;
        if (global_counter_ms - last_time_motion < 2000)
            return;

        last_time_motion = global_counter_ms;

        static int8_t lock_target = 0;

        if (gesture == 1)
        {

            if (hand_position < -130)
            {
                motion_to_point_interupt(final_pose_x, final_pose_y, final_pose_yaw + (-hand_position * 0.001f));
                lock_target = 0;

                logger.info("Hand left (pos=%d) → rotating left", hand_position);
            }
            else if (hand_position > 130)
            {
                motion_to_point_interupt(final_pose_x, final_pose_y, final_pose_yaw + (-hand_position * 0.001f));
                lock_target = 0;

                logger.info("Hand right (pos=%d) → rotating right", hand_position);
            }
            else
            {
                if (!lock_target)
                {
                    // (0.03, -6.46)
                    float target_x = obs_front_far.pos_x;
                    float target_y = obs_front_far.pos_y;

                    float angle_to_point = atan2f(target_y - final_pose_y, target_x - final_pose_x);

                    logger.info(" point (%.2f, %.2f) | robot (%.2f, %.2f)", target_x, target_y, final_pose_x, final_pose_y);

                    motion_to_point_interupt(cos(final_pose_yaw) * 3.0f + final_pose_x,
                                             sin(final_pose_yaw) * 3.0f + final_pose_y,
                                             final_pose_yaw);
                    lock_target = 1;
                }
            }
    ...
    void just_one_motion(int8_t gesture)
    {
        static int8_t prev_gesture = -1;
        static uint8_t counter_stop = 0;
        static float target_yaw = 0.0f;
        static float target_yaw_start = 0.0f;
        const int16_t tolerance = 160;

        static MachineState fsm_gesture;
        fsm_gesture.reentry(0, 0.8f);

        logger.info("Just one motion %d | %d %d", fsm_gesture.value, used_hand_position, counter_stop);
        logger.info("obs far status: %d dist: %.2f angle: %.2f", obs_front_far.status, obs_front_far.distance, obs_front_far.angle);
        // target_yaw_start = final_pose_yaw;

        switch (fsm_gesture.value)
        {
        case 0:
            motion_cancel();
            send_stop_command();
        ...
        case 3:
        {
            ...
            {
                counter_stop = 0;
                motion_to_point(cos(target_yaw) * (obs_front_far.distance - 0.3f) + final_pose_x,
                                sin(target_yaw) * (obs_front_far.distance - 0.3f) + final_pose_y,
                                target_yaw);
    ...
    void callback_ui_goto_docking(std_msgs::msg::Int8::SharedPtr msg)
    {
        motion_cancel();
        if (msg->data == 1)
        {
            logger.info("UI requested to go to docking station");
            fsm_robot.value = FSM_DOING_CHARGING;
        }
        else
        {
            logger.info("UI requested to cancel go to docking station");
            fsm_robot.value = FSM_SAFEOP;
        }
    }
    ...
    void callback_ds4(const ds4_driver_msgs::msg::Status::SharedPtr msg) {
        counter_kirim_motion_0 = 0;
        last_time_connected_ds4 = global_counter_ms;
    ...
    {
        // logger.info("Getting motion navigation status from Reeman controller...");

        cpr::Response res = cpr::Get(cpr::Url(build_url(ENDPOINT_GET_NAV_STATUS)));
        nlohmann::json json_res = nlohmann::json::parse(res.text);

        logger.info("Response from GET_NAV_STATUS: %s", res.text.c_str());
        nav_status[0] = json_res["res"];
        nav_status[1] = json_res["reason"];
        *error_position_m = json_res["dist"];

        // logger.info("dist_to_goal: %.2f, state: %d, code: %d", *error_position_m, nav_status[0], nav_status[1]);
    }

    void get_lidar_data()
    {
        // 1. Ambil data LiDAR dari REST
        cpr::Response res = cpr::Get(cpr::Url(build_url(ENDPOINT_GET_LASER_DATA)));
        if (res.status_code != 200)
        {
            logger.warn("Failed to fetch LiDAR data");
            return;
        }

        nlohmann::json json_res = nlohmann::json::parse(res.text);
        const auto &coords_json = json_res["coordinates"];

        std::vector<std::pair<float, float>> coords;
        for (const auto &p : coords_json)
            coords.emplace_back(p[0], p[1]);

        // // 2. Pose robot
        // get_robot_global_data();
        Pose2D pose{final_pose_x, final_pose_y, final_pose_yaw};

        // 3. Definisikan beberapa area deteksi (dalam frame lokal robot)
        std::vector<point2d_t> area_front = {
            {0.0f, -0.2f}, {0.0f, 0.2f}, {1.259f, 0.2f}, {1.259f, -0.2f}};

        std::vector<point2d_t> area_front_far = {
            {0.0f, -0.3f}, {0.0f, 0.3f}, {5.0f, 0.3f}, {5.0f, -0.3f}};

        // 4. Jalankan deteksi di tiap area
        obs_front_near = checkLidarAreaRect(coords, pose, area_front);
        obs_front_far = checkLidarAreaRect(coords, pose, area_front_far);
        // obs_cone_near = checkLidarAreaCone(coords, pose);
    }

    obstacle_checking_t checkLidarAreaCone(const std::vector<std::pair<float, float>> &coords,
                                           const Pose2D &robot_pose)
    {
        // get the first obstacle point in front of the robot within a cone area
        // first checking the 0 angle, then 1, then -1, then 2, then -2, etc.
        obstacle_checking_t result;
        float min_dist = FLT_MAX;
        ;
        bool found = false;

        for (int angle_deg = 0; angle_deg <= 5; angle_deg++)
        {
            for (int sign = -1; sign <= 1; sign += 2)
            {
                int check_angle = sign * angle_deg;
                float check_angle_rad = check_angle * M_PI / 180.0f;
                ...
                for (const auto &p : coords)
                {
                    ...
                    float point_angle = atan2f(y_robot, x_robot);

                    // Cek apakah titik berada dalam sudut kerucut
                    if (fabsf(point_angle - check_angle_rad) < (5.0f * M_PI / 180.0f)) // 5 degrees tolerance
                    {
    ...
    bool motion_to_docking_station()
    {
        static uint8_t docking_status = 0;

        if (docking_status == 0 || docking_canceled == true)
        {
            cpr::Response res = cpr::Post(cpr::Url(build_url(ENDPOINT_GO2CHARGE)),
                                          cpr::Body("{\"type\": 2, \"point\":\"Charging_Pile\"}"),
                                          cpr::Header{{"Content-Type", "application/json"}});

            sleep(0.500);

            docking_status = 1;
            docking_canceled = false;

            return false;
        }
        else if (docking_status == 1)
        {
            logger.info("Go to Moving to docking station...");
    ...
    {
        static float prev_target_x = target_x;
        static float prev_target_y = target_y;
        static float prev_target_yaw = target_yaw;

        static uint8_t motion_to_point_status = 0;

        static uint16_t timer_reentry = 0;
        ...
        if (prev_target_x != target_x || prev_target_y != target_y || prev_target_yaw != target_yaw)
        {
            logger.info("New target received, resetting motion_to_point status");
        ...
        if (motion_to_point_status == 0 || motion_to_point_canceled)
        {
            ...
            motion_to_point_canceled = false;
        }
        else if (motion_to_point_status == 1)
        {
            ...
            float target_error_angular = fabs(final_pose_yaw - target_yaw);

            while (target_error_angular > M_PI)
                target_error_angular -= 2 * M_PI;
            while (target_error_angular < -M_PI)
                target_error_angular += 2 * M_PI;

            printf("Target_error_position: %.2f | Target_error_angular: %.2f\n", target_error_position, target_error_angular);

            if (fabs(target_error_position) < position_error_tolerance && fabs(target_error_angular) < angle_error_tolerance)
            {
                motion_to_point_status = 0;
                motion_cancel();
                send_stop_command();
    ...
    void motion_to_point_interupt(float target_x, float target_y, float target_yaw, float max_speed = 0.4, float position_error_tolerance = 0.5, float angle_error_tolerance = 0.5)
    {

        logger.info("target x: %.2f, y: %.2f, yaw: %.2f || (%.2f %.2f %.2f)", target_x, target_y, target_yaw, final_pose_x, final_pose_y, final_pose_yaw);
    ...
    bool motion_cancel()
    {
        ...
        motion_to_point_canceled = true;
        docking_canceled = true;
    ...
    void get_robot_global_data() {
        ...
        // logger.info("Getting robot global data from Reeman controller...");
        cpr::Response res = cpr::Get(cpr::Url(build_url(ENDPOINT_GET_POSE)));

        if (res.status_code != 200)
        {
            logger.warn("Failed to fetch robot pose data");
            return;
        }

        nlohmann::json json_res = nlohmann::json::parse(res.text);

        // logger.info("Response from GET_POSE: %s", res.text.c_str());
        final_pose_x = json_res["x"];
        final_pose_y = json_res["y"];
        final_pose_yaw = json_res["theta"];

        // logger.info("Final Pose: x=%.2f, y=%.2f, yaw=%.2f", final_pose_x, final_pose_y, final_pose_yaw);

        res = cpr::Get(cpr::Url(build_url(ENDPOINT_GET_PWR_MGMT)));
        json_res = nlohmann::json::parse(res.text);
        battery_soc = json_res["battery"];
        emergency_button = json_res["emergencyButton"];
        charging_status = json_res["chargeFlag"];

        // logger.info("Response from GET_PWR_MGMT: %s", res.text.c_str());

        res = cpr::Get(cpr::Url(build_url(ENDPOINT_GET_MODE)));
        json_res = nlohmann::json::parse(res.text);
        robot_current_mode = json_res["mode"];

        res = cpr::Get(cpr::Url(build_url(ENDPOINT_GET_SPEED)));
        json_res = nlohmann::json::parse(res.text);
        final_vx = json_res["vx"];
        final_vth = json_res["vth"];

        // logger.info("Response from GET_MODE: %s", res.text.c_str());

        // logger.info("Robot current mode: %d", robot_current_mode);
        // logger.info("Final Pose: x=%.2f, y=%.2f, yaw=%.2f", final_pose_x, final_pose_y, final_pose_yaw);
        // logger.info("Battery SOC: %.2f%%, Emergency Button: %d", battery_soc, emergency_button);

        // get_motion_nav_status(current_robot_config.nav_status, &current_robot_config.distance_to_goal);
    }

    uint8_t isLeft(point2d_t p1, point2d_t p2, point2d_t p)
    {

        return (p2.x - p1.x) * (p.y - p1.y) - (p.x - p1.x) * (p2.y - p1.y) > 0;
    }

    void callback_routine_normal()
    {
        //? SOLVE DISPLAY
        static uint8_t set_display_mode = 0;

        if (set_display_mode == 0 && normal_global_counter_ms > 10000)
        {
            logger.info("Setting DISPLAY mode for xdotool...");
            system("export DISPLAY=:0 && sleep 1 && xdotool mousemove 500 300 click 1");
            set_display_mode = 1;
        ...
        normal_global_counter_ms += 200;
    }

    void callback_routine()
    {

        static int8_t prev_fsm_robot = 0;
        ...
        if (global_counter_ms - last_time_connected_ds4 > 2000)
        {
            ...
            {
                logger.warn("KIRIM STOP DS PUTUS");
                if (counter_kirim_motion_0 > 100)
                    counter_kirim_motion_0 = 100;
                motion_cancel();
                send_stop_command();
            }
        }
        ...
        logger.info("%d %d %d %d %d", fsm_robot.value, robot_operation_status, robot_current_mode, request_to_start, request_to_stop);
        ...
        case FSM_SAFEOP:
        {
            // logger.info("FSM_SAFEOP State");

            if (request_to_start)
            {
                if (current_terminals.size() > 0)
                {
        ...
        case FSM_SAFEOP_HANDTRACK:
        {
            just_one_motion(used_hand_gesture);

            break;
        }

        case FSM_HUMAN_INTERVENTION:
        {
            motion_cancel();
            send_stop_command();

            static uint64_t last_time_human_intervension = global_counter_ms;

            if (face_detected == 1)
            {
                last_time_human_intervension = global_counter_ms;
            }

            if (global_counter_ms - last_time_human_intervension > 3000)
            {
                logger.warn("Skipping request, not enough time has passed since the last request");
                fsm_robot.value = FSM_SAFEOP;
            }
        ...
        case FSM_OPERATION:
        {
            ...
            // motion_to_point(current_terminals[1].target_pose_x,
            //                 current_terminals[1].target_pose_y,
            //                 current_terminals[1].target_pose_theta,
            //                 current_terminals[1].target_max_velocity_x,
            //                 current_terminals[1].toleransi_posisi,
            //                 current_terminals[1].toleransi_sudut);
            break;
        }
        ...
        case FSM_DOING_CHARGING:
        {
            if (charging_status == 2)
            {
                pub_docking_status->publish(std_msgs::msg::Int8().set__data(1));
                fsm_robot.value = FSM_SAFEOP;
            }
            else
            {
                motion_to_docking_station();
            }
            break;
        }
        ...
        send_robot_battery_status();
        send_robot_velocity_status();

        prev_fsm_robot = fsm_robot.value;
        global_counter_ms += 100;
    ...
    void ds4_handler() {
        ...
        //     robot_operation_status = 1;
        // }
        if (ds4_current_status.button_cross > 0 && ds4_previous_status.button_cross == 0 && request_to_start == false)
        {
            request_to_start = true;
            request_to_stop = false;
        }
        else if (ds4_current_status.button_cross > 0 && ds4_previous_status.button_cross == 0 && request_to_start == true)
        {
            request_to_start = false;
        ...
        if (ds4_current_status.button_triangle > 0 && ds4_previous_status.button_triangle == 0 && fsm_robot.value != FSM_SAFEOP_HANDTRACK)
        {
            // fsm_robot.value = FSM_SAFEOP_HANDTRACK;
        }
        else if (ds4_current_status.button_square > 0 && ds4_previous_status.button_square == 0 && fsm_robot.value == FSM_SAFEOP_HANDTRACK)
        {
            fsm_robot.value = FSM_SAFEOP;
        }

        if (ds4_current_status.button_r1 > 0 && ds4_current_status.button_square > 0)
        {
            motion_cancel();
            std_msgs::msg::Int8 msg_hand_audio;
            msg_hand_audio.data = 1;
            ...
            // if (motion_to_point(target_point_1_x, target_point_1_y, target_point_1_yaw, 0.6, 0.5, 0.3))
            // {
            //     motion_cancel();
            // }
        }

        if (ds4_current_status.button_r1 > 0 && ds4_current_status.button_triangle > 0)
        {
            // motion_cancel();
            std_msgs::msg::Int8 msg_halo_audio;
            msg_halo_audio.data = 1;
            ...
            // logger.info("Reached Target Point 2");
            // if (motion_to_point(target_point_2_x, target_point_2_y, target_point_2_yaw, 0.6, 0.5, 0.3))
            // {
            //     motion_cancel();
            // }
        }

        if (ds4_current_status.button_l1 > 0 && ds4_current_status.button_r1 > 0)
        {
            fsm_robot.value = FSM_LAUNCHING;
        }
    }
    ...
    void send_robot_battery_status()
    {
        static uint32_t last_battery_status_publish_time_ms = 0;
        uint32_t current_time_ms = this->now().nanoseconds() / 1000000;
        if (current_time_ms - last_battery_status_publish_time_ms < 30000)
        {
            return;
        }

        std_msgs::msg::Float32 battery_msg;
        battery_msg.data = battery_soc;
        pub_robot_battery_status->publish(battery_msg);
    }

    void send_robot_velocity_status()
    {
        std_msgs::msg::Int8 msg_moving_status;
        static int cntr_robot_stop = 30;

        // msg_moving_status.data = 1;

        if (emergency_button == 1)
        {
            if (fabsf(final_vx) < 0.03f && fabsf(final_vth) < 0.3f)
            {
                if (cntr_robot_stop++ < 30)
                {
                    msg_moving_status.data = 1;
                }
                else
                {
                    msg_moving_status.data = 0;
                }
            }
            else
            {
        ...
        pub_moving_status->publish(msg_moving_status);
    }
    ...
    void motion_follow_routes() {
        ...
        logger.info("Current route idx: %d Routes size: %d", current_robot_config.current_route_index, current_robot_config.routes.size());
        if (change_terminals > 0) {
            for (const auto& terminal : current_terminals) {
                if (terminal.terminal_id == current_robot_config.routes[current_robot_config.current_route_index]) {
                    ...
                    {
                        current_robot_config.next_terminal = current_robot_config.routes[0];
                        current_robot_config.current_route_index = 0;
                    }
                    break;
                }
        ...
        logger.info("robot_pose_x: %.2f, robot_pose_y: %.2f, robot_pose_theta: %.2f", final_pose_x, final_pose_y, final_pose_yaw);
        ...
        {
            logger.info("Reached target terminal ID: %d", current_robot_config.current_terminal);
            change_terminals = 1;

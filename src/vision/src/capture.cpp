...
public:
    ...
    // /dev/v4l/by-id/usb-046d_HD_Webcam_C615_391DC610-video-index0
    std::string camera_path = "/dev/v4l/by-id/usb-046d_C922_Pro_Stream_Webcam_3BD7DCCF-video-index0";
    ...
    void callback_routine()
    {
        ...
        static int count = 0;
        count++;
        if (count % 6 != 0) return; // publish every 10th frame
